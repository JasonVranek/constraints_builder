...
use crate::{
    ...
    live_builder::{
        constraint_input::{
            start_constraintpool_jobs, ConstraintInputConfig, ConstraintPoolCommand,
        },
    ...
    P: StateProviderFactory,
{
    ...
    pub constraint_input_config: ConstraintInputConfig,
    ...
    pub constraint_sender: mpsc::Sender<ConstraintPoolCommand>,
    pub constraint_receiver: mpsc::Receiver<ConstraintPoolCommand>,
    ...
    P: StateProviderFactory + Clone + 'static,
{
    ...
    pub async fn run(self, ready_to_build: Arc<AtomicBool>) -> eyre::Result<()> {
        ...
        let constraintpool_subscriber = {
            let (handle, sub) = start_constraintpool_jobs(
                self.constraint_input_config,
                self.global_cancellation.clone(),
        ...
        while let Some(payload) = payload_events_channel.recv().await {
            ...
            if let Some(block_ctx) = BlockBuildingContext::from_attributes(
                ...
                if constraint_reserved_gas > 0 {
                    info!(
                        reserved_gas = constraint_reserved_gas,
                        gas_available = block_ctx.gas_available,
                        block_gas_limit = block_ctx.evm_env.block_env.gas_limit,
                        "Reserved gas for constraints based on transaction gas limits"
                    );
                }
